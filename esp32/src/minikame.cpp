...
void MiniKame::walk(float steps, int T){
    int x_amp = 15;
    int z_amp = 20;
    int ap = 20;
    int hi = 10;
...
//void MiniKame::followLine( bool isActive ) {
    //while(isActive){
 //       while(sensorValue1 < 50 && sensorValue2 < 50){
 //           walk(1, 550);
 //           float sensorValue1 = analogRead();
 //           float sensorValue2 = analogRead();
 //       }
 //       if(sensorValue1 > 50) {
 //           while(sensorValue1 < 50) {
 //            turnR(5, 550);
 //       }
 //       else if (sensorValue2 > 50) {
 //           while(sensorValue2 < 50) {
 //           turnL(5, 550);
 //       }
 //   }
...
void MiniKame::setServo(int id, float target){
    float  angle = map((180-(target+trim[id])), 0, 180, SERVOMIN, SERVOMAX);
...
void MiniKame::execute(float steps, int period[8], int amplitude[8], int offset[8], int phase[8]){
    ...
    while (millis() < _final_time){
        ...
        //    output[i] = oscillator[i].refresh();
        //    angle[i] = map(output[i], 0, 180, SERVOMIN, SERVOMAX);
        //    pwm.setPWM(i, 0, angle[i] );
        }
        yield();
    }
}
